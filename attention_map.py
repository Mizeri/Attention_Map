...
def plot_attention_map(img_path, model, w1, w2, index, size=512, channels=2048, i=1):
    ...
    cam = attention_map(img_path, model, w1, w2, size=size, channels=channels)
    v_min = cam.min()
    v_max = cam.max()
    ...
    cb = plt.colorbar(p3, cax=color_ax, extend='max')
    return v_min, v_max
